...
app.service('gradientService', function() {
    ...
    this.RGBGradient = function(x,steps){
        colors = TinyGradient(x);
        colors = colors.rgb(steps);
        output = [];
        colors.forEach(function(d){
            ...
            temp = 'rgb(' + r + ',' + g + ',' + b + ')'
            temp = tinycolor(temp);
            output.push(temp.toHexString());

        })
        return output;
    }

    this.HSVGradient = function(x,steps){
        colors = TinyGradient(x);
        colors = colors.hsv(steps);
        output = [];
        colors.forEach(function(d){
            r = Math.round(d._r);
            g = Math.round(d._g);
